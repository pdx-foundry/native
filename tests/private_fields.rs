...
use pdx_native::internals::fields::{self, FieldInput, Value};
use pdx_native::{
...
fn public_fields_need_only_an_executable_and_reject_foreign_subjects() {
    ...
    let static_input: pdx_native::internals::discovery::StaticInput =
        serde_json::from_slice(discovery.input_bytes()).unwrap();
    let candidates = pdx_native::internals::discovery::candidates(&static_input.symbols);
