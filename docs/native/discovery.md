...
- **Scopes** (`Native::scopes`). Scope types are the bits of `NEventScope::GetScopeName`. The method runs `GetScopeTypeEnumFromToken` on every token value up to the largest literal token, which groups the keywords of each type. It does not use the config's alias groups.
