...
The SDK-535 declaration reader is a separate static method. On the installed M45 build, one release example run measured 1.39 seconds for effects (including the first catalog read) and 0.75 seconds for triggers on the same `Native`. The SDK-559 registry timings below do not apply to it. The SDK-536 language questions on the same `Native`, in one release example run, took 0.46 s for modifiers, 0.45 s for categories, 0.35 s for scopes and 0.38 s for scope links.
