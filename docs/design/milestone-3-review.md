...
| SDK-549: scope context | Scope availability observed during parsing | Create a blocking observation ticket. Start from the SDK-488 prototype's 14 live parser-scope checks (`engine-commands.md:8`) and verify what transfers to the supported build. Close SDK-549 only after its required fixture observations pass. |
