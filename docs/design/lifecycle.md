...
The public registry client starts and reaps the consumer-supplied supervisor command and manages its private pipes. The bounded registry acceptance is withheld for renewal after PR review; see [registry queries](live-observations.md). `investigation::{connect, serve}` is a separate `maintainer-tools` surface producing
