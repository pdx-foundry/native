...
| `command_grammar` | Implemented for M45-release effects and triggers | A declaration kind and registered command name | Concrete shared reader identity, partial child families, fixed keys, numeric child grammar and conditional reader ordering. Every property keeps unresolved evidence explicit; this is not full argument grammar or runtime meaning. A missing registered command gives `UnknownCommand`. |
