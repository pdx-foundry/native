#!/usr/bin/env python3
"""Explicit real-game candidate registry controls. Never substitutes a synthetic target."""
import argparse
...
def main():
    ...
    subprocess.run(['cargo', 'build', '--locked', '--release', '--features', 'maintainer-tools', '--example', 'registry-investigation'], cwd=ROOT, check=True)
    ...
    shutil.copyfile(ROOT / 'target/release/examples/registry-investigation', output / 'registry-investigation')
    ...
    try:
        for scenario in ([args.scenario] if args.scenario else SCENARIOS):
            ...
            try:
                run = subprocess.run([str(ROOT / 'target/release/examples/registry-investigation'), str(args.installation), str(output / scenario), args.registry, scenario], text=True, capture_output=True, timeout=240)
                ...
                public = subprocess.run(['cargo', 'run', '--quiet', '--example', 'registry-replay', '--', str(evidence), str(evidence / 'descriptor.ref.json')], cwd=ROOT, capture_output=True, text=True, check=True)
